...
function getIntersectionNode(headA: ListNode | null, headB: ListNode | null): ListNode | null {
  ...
  let longPoint = pointA === null ? headB : headA;
  let shortPoint = pointA === null ? headA : headB;
  let resumePoint = pointA === null ? pointB : pointA;

  while (resumePoint !== null) {
    resumePoint = resumePoint.next;
    longPoint = longPoint!.next;
  }

  while (longPoint && shortPoint) {
    // 终于看懂题目了..是两个链表汇总到一个链表分支, 所以判断节点相等,不是值相等
