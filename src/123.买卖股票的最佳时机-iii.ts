...
function maxProfit(prices: number[]): number {
  // DP 最多五种情况🆘
  /**
   * 1什么都不操作 2第一次持有 3第一次不持有 4第二次持有 5第二次不持有
   * 1. dp[i][j]及下标含义: j区间[0, 4]代表五种情况, i表示第i天, dp[i][j]表示拥有的最大金额
   * 2. dp递推公式: 见下文 和122. 差不多
   * 3. dp初始化: dp[0][0] = 0; dp[0][1] = -prices[0]; dp[0][2] = 0; dp[0][3] = -prices[0], dp[0][4] = 0;
   * 4. 递归: 从前往后
   */
